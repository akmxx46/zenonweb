...
export function HeroSection() {
  ...
  const [emblaRef, emblaApi] = useEmblaCarousel({ 
    loop: true, 
    duration: 45 // Bikin perpindahan slide lebih berat & elegan
  });
    ...
    <section className="pt-20 pb-4 px-3 sm:px-6 bg-zinc-950">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-900 shadow-xl mb-3">
            ...
            <div className="flex">
              {SLIDES.map((slide, index) => (
                <div className="flex-[0_0_100%] min-w-0 relative h-[230px] sm:h-[320px] flex items-center p-6 sm:p-12" key={index}>
                  <img 
                    src={slide.bg} 
                    className={`absolute inset-0 w-full h-full object-cover opacity-90 transition-transform duration-[7000ms] ease-out ${selectedIndex === index ? 'scale-110' : 'scale-100'}`} 
                    alt="" 
                  />
          ...
          {/* --- PAGINATION DOTS (SMOOTH MORPHING) --- */}
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20">
            {SLIDES.map((_, index) => (
              <button
                ...
                style={{
                    // Container ikut melebar biar transisinya fluid
