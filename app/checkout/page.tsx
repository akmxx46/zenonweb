...
function CheckoutContent() {
  ...
  // States untuk data pembeli (Email sudah dihapus)
  ...
  const handlePayment = async () => {
    ...
    // Validasi tambahan jika produk adalah panel
    ...
    // Format Pesan WhatsApp (Email dihapus dari sini juga)
    const message = `Halo Admin ZenonStore,%0A%0A` +
                    `Saya ingin order *${productType === "panel" ? "Pterodactyl" : "Script/App"}*:%0A` +
                    ...
                    `Mohon instruksi selanjutnya dan kode QRIS-nya ya Min!`;
    ...
    <div className="min-h-screen bg-zinc-950 py-6 sm:py-8 px-3 sm:px-4 lg:px-6 text-zinc-100">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-zinc-400 text-sm">Proses cepat tanpa ribet</p>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-white font-medium text-sm sm:text-base">
                {productType === "panel" ? `Pterodactyl ${productName}` : productName}
              </p>
              <p className="text-zinc-500 text-xs sm:text-sm">
                {productType === "panel" ? "Aktivasi Instan" : "Produk Digital"}
              </p>
            </div>
        ...
        <div className="bg-zinc-900/90 border border-zinc-700/80 rounded-xl p-4 sm:p-5 mb-4 sm:mb-6">
          ...
          <div className="space-y-4">
            ...
            {/* INPUT USERNAME PANEL (Khusus Panel) */}
            {productType === "panel" && (
              <div className="p-4 bg-teal-500/5 border border-teal-500/20 rounded-xl">
                ...
                <input
                  ...
                  placeholder="Contoh: user_pteronode"
                ...
                />
                <p className="text-[10px] text-zinc-500 mt-2 italic">*Digunakan untuk akses login panel Anda nantinya</p>
              </div>
            )}
          </div>
        </div>
        ...
        <p className="text-center text-zinc-500 text-[10px] mt-4 italic">
          * Klik untuk mendapatkan instruksi pembayaran QRIS dari admin
        </p>
      </div>
    </div>
  );
...
export default function CheckoutPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center text-teal-400">
